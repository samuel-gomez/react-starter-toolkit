...
describe('<MenuEnhanced />', () => {
  ...
  it('Render <Menu/>', () => {
    const { asFragment } = renderWithWrapperStaticRouter(<MenuEnhanced menuItems={MENU_ITEMS_MOCK} isVisible={false} onClick={onClickFn} />);
    expect(asFragment()).toMatchSnapshot();
  });

  it('Render <Menu /> fullscreen', () => {
    const { asFragment } = renderWithWrapperStaticRouter(
      <MenuEnhanced menuItems={MENU_ITEMS_MOCK} fullScreen isVisible={false} onClick={onClickFn} />,
    );
    expect(asFragment()).toMatchSnapshot();
  });

  it('Should call toggle class body When click on button', () => {
    renderWithWrapperStaticRouter(<MenuEnhanced menuItems={MENU_ITEMS_MOCK} isVisible={false} onClick={onClickFn} />);
