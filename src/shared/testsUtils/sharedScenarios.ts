...
export const UnChampTextEstVisibleAvecLaValeurEtUnPlaceholder = (instruction: DefineStepFunction) =>
  instruction(/^un champ texte "(.*)" est visible avec la valeur "(.*)", un placeholder "(.*)"$/, (name, value, placeholder) => {
    expect(screen.getByRole('textbox', { name: RegExp(name) })).toHaveAttribute('value', value);
    expect(screen.getByRole('textbox', { name: RegExp(name) })).toHaveAttribute('placeholder', placeholder);
  });
...
export const UnChampTextEstVisible = (instruction: DefineStepFunction) =>
  instruction(/^un champ texte "(.*)" est visible avec la valeur "(.*)"$/, (fieldName, value) => {
    expect(screen.getByRole('textbox', { name: RegExp(fieldName) })).toHaveValue(value);
  });

export const UnChampTextEstMasque = (instruction: DefineStepFunction, parentLabel = '') =>
  instruction(/^un champ texte "(.*)" est masqué$/, fieldName => {
    const base = !!parentLabel ? within(screen.getByLabelText(parentLabel)) : screen;
    expect(base.queryByRole('textbox', { name: RegExp(fieldName) })).not.toBeInTheDocument();
  });

export const JeSaisieDansLeChamp = (instruction: DefineStepFunction, parentLabel = '') =>
  instruction(/^je saisie "(.*)" dans le champ "(.*)"$/, (value, fieldName) => {
    const base = !!parentLabel ? within(screen.getByLabelText(parentLabel)) : screen;
    const input = base.getByRole('textbox', { name: fieldName });
...
export const JeCliqueSurLaCheckbox = async (instruction: DefineStepFunction, parentLabel = '') =>
  instruction(/^je clique sur la checkbox "(.*)"$/, async label => {
    const base = !!parentLabel ? within(screen.getByLabelText(parentLabel)) : screen;
    const inputCheckboxtoggle = base.getByRole('checkbox', { name: label });
    userEvent.click(inputCheckboxtoggle);
  });
...
export const UnChampTextALeFocus = (instruction: DefineStepFunction, parentLabel = '') =>
  instruction(/^un champ texte "(.*)" a le focus$/, fieldName => {
    const base = !!parentLabel ? within(screen.getByLabelText(parentLabel)) : screen;
    const input = base.getByRole('textbox', { name: RegExp(fieldName) });
    expect(input).toHaveFocus();
  });

export const UnChampTextEstDesactive = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstActive = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstOptionnel = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstRequis = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstEnLectureSeule = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstEditable = (instruction: DefineStepFunction, parentLabel = '') =>
...
export const UnChampTextEstVisibleAvecSonWrapper = (instruction: DefineStepFunction, parentLabel = '') =>
