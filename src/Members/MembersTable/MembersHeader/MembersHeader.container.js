...
export const MembersHeaderEnhanced = ({ setSortingInfoFn, headers }) => {
  const { onChangeSorting, sorting } = useContext(MembersContext);
  const { field, order } = sorting;
  const sortingInfo = setSortingInfoFn({ field, order });

  return <MembersHeader sorting={sorting} sortingInfo={sortingInfo} onSort={onChangeSorting} headers={headers} />;
};
